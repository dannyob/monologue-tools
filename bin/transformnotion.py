...
class URLRewritingMarkdownRenderer(MarkdownRenderer):
    ...
    def render_link(self, element: inline.Link) -> str:
        ...
        link_dest, page_title, page_content = get_redirected_url_and_content(
            element.dest
        )
        if link_dest == element.dest:
            ...
            self.missing_links.append(
                (link_text, element.dest, context_element, page_title, page_content)
            )
...
def transform_markdown(file):
    ...
    for (
        ...
        page_title,
        page_content,
    ) in transforming_renderer.missing_links:
        context = extract_paragraph_text(paragraph_element)
        processed_missing_links.append(
            (link_text, notion_url, context, page_title, page_content)
        )
